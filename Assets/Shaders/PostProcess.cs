...
{
    public static RenderBuffer[] mainRenderBuffers;
    public static RenderTexture mainRenderTexture0;
    public static RenderTexture mainRenderTexture1;
    public static RenderTexture mainRenderTextureA;
    public static RenderTexture mainRenderTextureB;
    ...
    private RenderTexture opacityTarget;
    private Texture2D opacityTexture;

    private RenderTexture distanceFieldTarget0;
    private Texture2D distanceFieldTexture0;

    private RenderTexture distanceFieldTarget1;
    private Texture2D distanceFieldTexture1;

    private Rect distanceFieldRect;
    private float[] distances;
    private ivec2[] vectors;
    private bool[] closed;
    private byte[] filled;
    ...
    private PostUtils.PriorityQueue<ivec4, float> pq = new();

    private bool computeSDF = true;
    private bool useGPU = true;
    private float sdfMin = 0;
    private float sdfSize = 1;

    private RenderTexture[] targets = new RenderTexture[2];

    void Awake()
    {
        opacityMaterial = new Material(Shader.Find("Hidden/KCG_OpacityMap"));
        distanceFieldMaterial = new Material(Shader.Find("Hidden/KCG_SignedDistanceField"));
        compositeMaterial = new Material(Shader.Find("Hidden/KCG_Composite"));
    }
    ...
    void Update()
    {
        if (Camera.current != null)
        {
            ...
            {
                mainRenderTexture0 = new RenderTexture(width, height, UnityEngine.Experimental.Rendering.GraphicsFormat.R16G16B16A16_SFloat, UnityEngine.Experimental.Rendering.GraphicsFormat.D24_UNorm_S8_UInt);
                mainRenderTexture1 = new RenderTexture(width, height, UnityEngine.Experimental.Rendering.GraphicsFormat.R16G16B16A16_SFloat, UnityEngine.Experimental.Rendering.GraphicsFormat.None);
                mainRenderBuffers = new RenderBuffer[2];
                mainRenderBuffers[0] = mainRenderTexture0.colorBuffer;
                mainRenderBuffers[1] = mainRenderTexture1.colorBuffer;
                mainRenderTextureA = new RenderTexture(width, height, UnityEngine.Experimental.Rendering.GraphicsFormat.R16G16B16A16_SFloat, UnityEngine.Experimental.Rendering.GraphicsFormat.D24_UNorm_S8_UInt);
                mainRenderTextureB = new RenderTexture(width, height, UnityEngine.Experimental.Rendering.GraphicsFormat.R16G16B16A16_SFloat, UnityEngine.Experimental.Rendering.GraphicsFormat.None);

                distanceFieldTarget0 = new RenderTexture(width, height, UnityEngine.Experimental.Rendering.GraphicsFormat.R16G16B16A16_SFloat, UnityEngine.Experimental.Rendering.GraphicsFormat.None);
                distanceFieldTexture0 = new Texture2D(width, height, TextureFormat.RGBAHalf, false);

                distanceFieldTarget1 = new RenderTexture(width, height, UnityEngine.Experimental.Rendering.GraphicsFormat.R16G16B16A16_SFloat, UnityEngine.Experimental.Rendering.GraphicsFormat.None);
                distanceFieldTexture1 = new Texture2D(width, height, TextureFormat.RGBAHalf, false);

                opacityTarget = new RenderTexture(width, height, UnityEngine.Experimental.Rendering.GraphicsFormat.R16G16B16A16_SFloat, UnityEngine.Experimental.Rendering.GraphicsFormat.None);
                opacityTexture = new Texture2D(width, height, TextureFormat.RGBAHalf, false);

                distanceFieldRect = new Rect(0, 0, width, height);
                distances = new float[width * height];
                vectors = new ivec2[width * height];
    ...
    {
        //source = mainRenderTexture0;
        if (opacityTexture != null)
        {
            if (useGPU)
            {
                float beta = 1;
                var sdfParams = new UnityEngine.Vector4();

                var rt = UnityEngine.RenderTexture.active;
                UnityEngine.RenderTexture.active = distanceFieldTarget0;
                GL.Clear(true, true, Color.black);
                UnityEngine.RenderTexture.active = distanceFieldTarget1;
                GL.Clear(true, true, Color.black);
                UnityEngine.RenderTexture.active = opacityTarget;
                GL.Clear(true, true, Color.black);
                UnityEngine.RenderTexture.active = rt;



                //var pixelMaterial = new UnityEngine.Material(UnityEngine.Shader.Find("Hidden/PixelShader"));
                //pixelMaterial.SetFloat("_PixelWidth", 1);
                //pixelMaterial.SetFloat("_PixelHeight", 1);
                //pixelMaterial.SetFloat("_ScreenWidth", mainRenderTexture0.width);
                //pixelMaterial.SetFloat("_ScreenHeight", mainRenderTexture0.height);
                //UnityEngine.Graphics.Blit(mainRenderTexture0, mainRenderTextureA, pixelMaterial);
                //pixelMaterial.SetFloat("_ScreenWidth", mainRenderTexture1.width);
                //pixelMaterial.SetFloat("_ScreenHeight", mainRenderTexture1.height);
                //UnityEngine.Graphics.Blit(mainRenderTexture1, mainRenderTextureB, pixelMaterial);


                mainRenderTextureA = mainRenderTexture0;
                mainRenderTextureB = mainRenderTexture1;

            float tapCount = 64;

                targets[0] = distanceFieldTarget0;
                targets[1] = opacityTarget;
                opacityMaterial.SetTexture("_KCG_Material", mainRenderTextureB);
                opacityMaterial.SetVector("_KCG_Invert", new UnityEngine.Vector4(0, 0, 0, 0));
                Graphics.Blit(mainRenderTextureA, targets[0], opacityMaterial);
                Graphics.Blit(mainRenderTextureA, targets[1], opacityMaterial);
                for (int i = 0; i < tapCount; i++)
                {
                    sdfParams.x = beta;
                    sdfParams.y = 1.0f / width;
                    sdfParams.z = 0;
                    distanceFieldMaterial.SetVector("_KCG_SDF_Params", sdfParams);
                    Graphics.Blit(targets[((i + 1) % 2)], targets[i % 2], distanceFieldMaterial);
                    beta += 2;
                }
                beta = 1;
                for (int i = 0; i < tapCount; i++)
                ...
                targets[0] = distanceFieldTarget1;
                targets[1] = opacityTarget;
                opacityMaterial.SetTexture("_KCG_Material", mainRenderTextureB);
                opacityMaterial.SetVector("_KCG_Invert", new UnityEngine.Vector4(1, 1, 1, 1));
                Graphics.Blit(mainRenderTextureA, targets[0], opacityMaterial);
                Graphics.Blit(mainRenderTextureA, targets[1], opacityMaterial);
                for (int i = 0; i < tapCount; i++)
                {
                ...
                compositeMaterial.SetVector("_KCG_Viewport", new UnityEngine.Vector4(Camera.main.pixelWidth, Camera.main.pixelHeight, 0, 0));
                compositeMaterial.SetVector("_KCG_SDF_Scale", new UnityEngine.Vector4(sdfMin, sdfSize, Camera.main.orthographicSize, 0));
                compositeMaterial.SetTexture("_KCG_Material", mainRenderTextureB);
                compositeMaterial.SetTexture("_KCG_DistanceField0", distanceFieldTarget0);
                compositeMaterial.SetTexture("_KCG_DistanceField1", distanceFieldTarget1);
                Graphics.Blit(mainRenderTextureA, destination, compositeMaterial);

                distanceFieldTarget0.DiscardContents();
                opacityTarget.DiscardContents();

                //RenderTexture.active = destination;

                Camera.current.SetTargetBuffers(Graphics.activeColorBuffer, Graphics.activeDepthBuffer);


            }
            //else
            //{
            //    Graphics.Blit(source, opacityTarget, opacityMaterial);

            //    if (computeSDF)
            //    {

            //        RenderTexture.active = opacityTarget;
            //        opacityTexture.ReadPixels(distanceFieldRect, 0, 0, false);
            //        RenderTexture.active = null;

            //        filled = opacityTexture.GetRawTextureData();
            //        var count = filled.Length;

            //        ComputeSDF();

            //        //sdfMin = float.MaxValue;
            //        //float max = float.MinValue;
            //        //for (int i = 0; i < count; i++)
            //        //{
            //        //    sdfMin = Math.Min(distances[i], sdfMin);
            ...
            //        //for (int i = 0; i < count; i++)
            //        //{
            //        //    filled[i] = (byte)(255 * ((distances[i] - sdfMin) / sdfSize));
            //        //}

            //        distanceFieldTexture0.SetPixelData(distances, 0);
            //        distanceFieldTexture0.Apply();
            //        computeSDF = false;
            //    }

            //    compositeMaterial.SetVector("_KCG_SDF_Scale", new UnityEngine.Vector4(sdfMin, sdfSize, Camera.main.orthographicSize, 0));
            //    compositeMaterial.SetTexture("_KCG_DistanceField", distanceFieldTarget0);
    ...
    bool is_filled(int x, int y, bool negate)
    {
        if (valid_pixel(x, y))
            return (filled[x + y * width] > 0) ^ negate;
        return false ^ negate;
    }
    void ComputeSDF()
    {
        ...
        Array.Clear(distances, 0, closed.Length);

        vectors[0].x = 2 * width + 1;
        vectors[0].y = 2 * height + 1;

        ComputePartialSDF(false);
        ComputePartialSDF(true);

        int count = vectors.Length;
        for (int i = 0; i < count; i++)
        {
            var d = vectors[i];
            distances[i] = MathF.Sqrt(d.x * d.x + d.y * d.y) / 2;
            if (filled[i] > 0)
                distances[i] = -distances[i];
        }
    }
    void ComputePartialSDF(bool negate)
    {
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < width; x++)
            {
                if (is_filled(x, y, negate))
                {
                        ...
                        for (int dx = -1; dx <= 1; dx++)
                        {
                            ...
                            if (valid_pixel(x2, y2))
                            {
                                if (!is_filled(x2, y2, negate))
                                {
                                    ...
                                    v.d.x = dx2;
                                    v.d.y = dy2;
                                    pq.Enqueue(v, MathF.Sqrt(dx2 * dx2 + dy2 * dy2));
                                }
                            }
                        }
                    }
        ...
        while (pq.TryDequeue(out var current, out var priority))
        {
            var index = current.x + current.y * width;
            if (closed[index])
                continue;

            closed[index] = true;
            vectors[index] = current.d;
            ...
            for (int dy = -1; dy <= 1; dy++)
            {
                for (int dx = -1; dx <= 1; dx++)
                {
                    ...
                    if (valid_pixel(x2, y2))
                    {
                        if (!is_filled(x2, y2, negate) && !closed[index2])
                        {
                            int dx2 = 2 * (x2 - current.x);
                            int dy2 = 2 * (y2 - current.y);
                            var dd = vectors[index];
                            dx2 += dd.x;
                            dy2 += dd.y;
