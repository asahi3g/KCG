    ...
    public class GoapAStar
    {
        ...
        private bool CreateActionPath(GoapState GoalState, Node Parent, Queue<int> ListofActions)
        {
            ...
            foreach (GameEntity action in NeighborActions)
            {
                ...
                {
                    ListofActions.Enqueue(action.nodeID.ID);
                    Node node = Parent;
                    while (node != RootNode)
                    {
                ...
                if (IsNodeOnList(OpenList, NewWorldState, NewPathCost, action.nodeID.ID))
                    continue;
                if (IsNodeOnList(ClosedList, NewWorldState, NewPathCost, action.nodeID.ID))
                    continue;

                // Add new node to OpenList.
                OpenList.Add(new Node(Parent, NewWorldState, action.nodeID.ID,
