...
namespace KMath.Noise 
{
    public class PerlinField2D 
    {
        ...
        public float GetNoiseSingle(float x, float y) 
        {
            ...
            float n00 = GradientArray[gi00].Dot(x, y);
            float n01 = GradientArray[gi01].Dot(x, y - 1);
            float n10 = GradientArray[gi10].Dot(x - 1, y);
            float n11 = GradientArray[gi11].Dot(x - 1, y - 1);
