namespace KMath.Noise
{
    public class PerlinField3D
    {
        ...
        public static readonly Vec3f[] GradientArray = new Vec3f[16] {
                ...
                new Vec3f( 1f,-1f, 0f).Normalize(),
                new Vec3f(-1f,-1f, 0f).Normalize(),
                new Vec3f( 1f, 0f, 1f).Normalize(),
                new Vec3f(-1f, 0f, 1f).Normalize(),
                new Vec3f( 1f, 0f,-1f).Normalize(),
                new Vec3f(-1f, 0f,-1f).Normalize(),
                new Vec3f( 0f, 1f, 1f).Normalize(),
                new Vec3f( 0f,-1f, 1f).Normalize(),
                new Vec3f( 0f, 1f,-1f).Normalize(),
                new Vec3f( 0f,-1f,-1f).Normalize(),
        ...
        public float GetNoiseSingle(float x, float y, float z)
        {
            ...
            float n000 = GradientArray[gi000].Dot(x, y, z);
            float n001 = GradientArray[gi001].Dot(x, y, z - 1);
            float n010 = GradientArray[gi010].Dot(x, y - 1, z);
            float n011 = GradientArray[gi011].Dot(x, y - 1, z - 1);
            float n100 = GradientArray[gi100].Dot(x - 1, y, z);
            float n101 = GradientArray[gi101].Dot(x - 1, y, z - 1);
            float n110 = GradientArray[gi110].Dot(x - 1, y - 1, z);
            float n111 = GradientArray[gi111].Dot(x - 1, y - 1, z - 1);
